...
class KVPParser:
        ...
        while True:
            ...
            if m.group('comment'):
                ...
                if isinstance(v, types.StringType):
                    v = v.decode(encoding)
                yield 'identifier', v
            elif m.group('integer'):
                yield 'integer', int(m.group('integer'))
            elif m.group('float1'):
                yield 'float', float(m.group('float1'))
            elif m.group('float2'):
                yield 'float', float(m.group('float2'))
            elif m.group('separator'):
                yield 'separator', m.group('separator')
            elif m.group('equalsign'):
                yield 'equalsign', m.group('equalsign')
                ...
                if isinstance(v, types.StringType):
                    v = v.decode(encoding)
                yield 'string', v
            elif m.group('trailingblanks'):
                yield 'trailingblanks', m.group('trailingblanks')
        if pos != len(text):
    ...
    def _innerWriteKVP(self, ofile, dct):
        for k, v in dct.items():
            if isinstance(v, types.StringTypes):
                if '"' in v:
                    ofile.write("%s = '%s'\n" % (k, v))  # preserve the quoted substring in v
                else:
                    ofile.write('%s = "%s"\n' % (k, v))
    ...
    def writeKVP(self, ofileOrFilename, dct, encoding=None):
        """
